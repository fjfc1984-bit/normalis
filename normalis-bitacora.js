...
function logAction(modulo, accion, detalle) {
  let logs = JSON.parse(localStorage.getItem('normalis_bitacora') || '[]');
  let user = 'Usuario';
  try {
...
function renderBitacora() {
  ...
  const filtered = logs.filter(l => {
    const matchSearch = !search || l.usuario.toLowerCase().includes(search) || l.accion.toLowerCase().includes(search) || l.detalle.toLowerCase().includes(search);
  ...
  tbody.innerHTML = page.map(l => {
    ...
    const color = modColors[l.modulo] || '#64748b';
...
function exportBitacoraCSV() {
  ...
  logs.forEach(l => {
    let d = new Date(l.ts).toLocaleString('es-CO');
...
function exportBitacoraPDF() {
  ...
  let logs = JSON.parse(localStorage.getItem('normalis_bitacora') || '[]').slice(0,100);
...
const _origSavePQRS = window.savePQRS;
window.savePQRS = function() {
  let tipo = document.getElementById('pqrs-tipo')?.value || '';
  let nombre = document.getElementById('pqrs-nombre')?.value || '';
  if (_origSavePQRS) _origSavePQRS();
  logAction('PQRS', 'Registró nueva PQRS', tipo + ' de ' + nombre);
};

const _origSaveInc = window.saveIncidente;
window.saveIncidente = function() {
  let tipo = document.getElementById('inc-tipo')?.value || '';
  const sev = document.getElementById('inc-severidad')?.value || '';
  if (_origSaveInc) _origSaveInc();
  logAction('Incidentes', 'Registró evento adverso', tipo + ' — severidad ' + sev);
};

const _origSaveVenc = window.saveVenc;
window.saveVenc = function() {
  const prof = document.getElementById('venc-profesional')?.value || '';
  let tipo = document.getElementById('venc-tipo')?.value || '';
...
window.saveNewProfesional = function() {
  let nombre = document.getElementById('prof-nombre')?.value || document.getElementById('new-prof-nombre')?.value || '';
