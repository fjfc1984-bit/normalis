# NormaLis — Scripts de Outreach
**Meta: 3 pilotos pagos en 30 días**

---

## SEMANA 1 — Lista de prospectos

**Ya no hay que armar esto a mano.** El CRM (`/admin/crm`) tiene un botón "Importar CSV" que carga
directo desde el REPS (dato abierto del Ministerio de Salud) — nombre, ciudad, email y teléfono ya
filtrados por IPS privadas en tus ciudades con tracción (Barranquilla, Santa Marta, Medellín, Popayán,
Ciénaga). El primer lote de 75 ya está importado en etapa "Nuevo", fuente "llamada fría".

Para cada contacto del CRM, antes de escribirle, marcar mentalmente:
- ¿Conoces a alguien ahí? → usar Mensaje 1A (caliente)
- ¿Te lo refirió alguien? → usar Mensaje 1B (tibio)
- Ninguno de los anteriores → usar Mensaje 1C (frío) — el botón de WhatsApp del CRM ya lo prellena.

---

## WHATSAPP — Mensajes listos para copiar

### Mensaje 1A — Contacto caliente (conocido)
> Hola [Nombre], ¿cómo estás?
>
> Te cuento que estoy lanzando NormaLis, una plataforma para que las IPS manejen la habilitación y el PAMEC sin tanto papel. Automatiza listas de chequeo, vencimientos y reportes según la Resolución 1732/2026.
>
> Sé que en [nombre IPS] siempre están pendientes de la habilitación — y con la 1732 recién publicada, todos están en plan de transición. ¿Te gustaría probarla gratis 30 días? Sin tarjeta, con soporte incluido.
>
> Te mando el detalle 👇

*[Adjuntar NormaLis-OnePager.pdf]*

---

### Mensaje 1B — Contacto tibio (referido)
> Hola [Nombre], lo saluda Fernando. Me pasaron su contacto porque trabaja en [nombre IPS] y andan muy juiciosos con el tema de habilitación.
>
> Lanzo NormaLis, una plataforma web que ayuda a las IPS a preparar auditorías de habilitación, gestionar el PAMEC e incidentes, todo según la Resolución 1732/2026.
>
> ¿Puedo enviarle información? Le tengo un piloto gratuito de 30 días sin ningún compromiso.

*[Esperar respuesta antes de enviar el PDF]*

---

### Mensaje 1C — Contacto frío
> Buenos días, [Nombre]. Le escribo de NormaLis, plataforma de cumplimiento normativo para IPS.
>
> ¿Ya tienen un plan de transición hacia la Resolución 1732/2026, la que reemplazó la 3100?
>
> Tenemos un piloto gratuito de 30 días que varias IPS en [ciudad] están evaluando esta semana.

*[Si responde sí: "¿Qué herramienta usan?" — si dice Excel/carpetas: continuar con Mensaje 2]*

---

### Mensaje 2 — Seguimiento (2 días después, sin respuesta)
> [Nombre], buen día. Solo quería confirmar que le llegó el mensaje anterior sobre NormaLis.
>
> Le comparto el one-pager por si le parece útil revisarlo.
>
> Si no es el momento, no hay problema — con gusto le escribo en otro momento.

*[Adjuntar NormaLis-OnePager.pdf]*

---

### Mensaje 3 — Cierre de piloto (después de una semana de prueba)
> [Nombre], ¿cómo le fue con NormaLis esta semana?
>
> ¿Pudo probar la auditoría o el chat normativo? Cuénteme qué le pareció para mejorar lo que necesite.
>
> Si quiere seguir después del piloto, el plan Starter está en $299.000/mes y cubre hasta 3 usuarios. ¿Lo activamos?

---

### Mensaje — Para grupos de WhatsApp (colegas / redes de salud)
> Buenos días a todos. Les comparto NormaLis, una plataforma que creé para ayudar a las IPS colombianas con la gestión de habilitación y PAMEC.
>
> Automatiza listas de chequeo (Res. 1732/2026), alertas de vencimientos, registro de incidentes y genera el reporte PDF para las visitas. Tiene IA para resolver preguntas normativas 24/7.
>
> Si trabajan en una IPS o conocen alguien que lo haga, tengo pilotos gratuitos de 30 días disponibles. Pueden registrarse en normalis.co o escribirme directamente.
>
> 🙏

---

## LINKEDIN — Posts para publicar

### Post 1 — Lanzamiento (publicar Día 1)

**¿Por qué las IPS colombianas siguen manejando la habilitación en Excel?**

Trabajé meses con equipos de calidad de IPS y encontré el mismo patrón:
- Carpetas físicas que se desactualizan solas
- Vencimientos que se pierden sin alertas
- Visitas de habilitación que generan pánico

La Resolución 1732/2026 acaba de reemplazar la 3100/2019 y todas sus modificaciones — mismos 7 estándares, pero con nuevos criterios de telemedicina e historia clínica electrónica interoperable, y solo 12 meses de transición ya corriendo. Mantenerse al día manualmente no es sostenible.

Por eso construí **NormaLis** — una plataforma 100% colombiana que automatiza:
✅ Listas de chequeo por servicio habilitado
✅ PAMEC integrado al flujo de auditoría
✅ Alertas de vencimientos
✅ Chat IA con respuestas normativas verificadas
✅ Reporte PDF listo para la visita

**Piloto gratuito de 30 días en normalis.co**

Si trabaja en una IPS o conoce quién lo haga — me encantaría que lo pruebe y me diga qué le falta.

#SaludColombia #HabilitacionIPS #Resolucion1732 #PAMEC #HealthTech

---

### Post 2 — Educativo (publicar Día 5)

**3 errores que cometen las IPS antes de la visita de habilitación:**

**1. Actualizar la documentación solo cuando viene la visita**
La habilitación es un proceso continuo, no un evento. Los criterios deben estar al día todo el año.

**2. Tener el PAMEC separado del proceso de auditoría**
El ciclo PHVA del PAMEC debe alimentarse directamente de los hallazgos de las auditorías internas. Si están en documentos distintos, se pierde el hilo.

**3. No tener trazabilidad de incidentes**
La Resolución 1732/2026 exige registro y análisis de incidentes. Sin un sistema, es imposible demostrar gestión ante los verificadores.

En NormaLis todo esto está conectado en una sola plataforma.

¿Cuál de estos errores ha visto en su IPS? 👇

#CalidadEnSalud #HabilitacionIPS #GestiónRiesgo

---

### Post 3 — Social proof (publicar Día 10, después de primeros pilotos)
*(Personalizar con nombre real del cliente si da permiso)*

**"Por primera vez, siento que tenemos la habilitación bajo control."**

Eso me dijo [Director de Calidad] de [nombre IPS] después de su primera semana usando NormaLis.

Antes: carpetas físicas, Excel, estrés antes de cada visita.
Ahora: listas de chequeo actualizadas, alertas automáticas, reporte listo en un clic.

Quedan **3 cupos de piloto gratuito** disponibles esta semana.

Si su IPS está buscando cómo simplificar la gestión de habilitación, escríbame o ingrese a normalis.co.

---

## EMAIL — Secuencia de 3 correos

### Email 1 — Primer contacto
**Asunto:** ¿Cómo gestiona actualmente la habilitación en [nombre IPS]?

Estimado/a [Nombre],

Le escribo porque trabajo con IPS colombianas que buscan simplificar la gestión de habilitación y PAMEC, y [nombre IPS] apareció en el REPS como una institución activa en [ciudad].

Lancé NormaLis, una plataforma web que automatiza las listas de chequeo de la Resolución 1732/2026, gestiona vencimientos, incidentes y PQRS, e incluye un chat con IA para resolver dudas normativas en cualquier momento.

Le adjunto un resumen de una página. Si le interesa, tengo disponible un piloto gratuito de 30 días — sin tarjeta, con soporte incluido.

¿Tiene 15 minutos esta semana para una llamada rápida?

Quedo atento,
Fernando
normalis.co | hola@normalis.co

*[Adjuntar NormaLis-OnePager.pdf]*

---

### Email 2 — Seguimiento (5 días después, sin respuesta)
**Asunto:** Re: ¿Cómo gestiona actualmente la habilitación en [nombre IPS]?

[Nombre], buen día.

Le reenvío mi mensaje anterior por si se perdió entre el correo.

Para contextualizar rápido: NormaLis es una plataforma para que las IPS manejen la habilitación de forma digital — listas de chequeo, PAMEC, incidentes y vencimientos, todo en un solo lugar según la Res. 1732/2026.

Esta semana tengo disponibles los últimos cupos de piloto gratuito de 30 días.

¿Le gustaría agendar una demostración de 20 minutos?

Saludos,
Fernando
normalis.co

---

### Email 3 — Último intento (5 días después)
**Asunto:** Último mensaje sobre NormaLis

[Nombre],

Entiendo que el tiempo es escaso. No le escribiré más si no es el momento.

Solo quería dejarle saber que el piloto gratuito de NormaLis sigue disponible en normalis.co — puede registrarse en cualquier momento sin necesidad de hablar conmigo primero.

Si en el futuro la gestión de habilitación se vuelve una prioridad, ahí estaré.

Mucho éxito con [nombre IPS].

Fernando

---

## DEMO — Guion de 20 minutos

### Apertura (2 min)
*"Gracias por su tiempo. Voy a mostrarle NormaLis en 20 minutos — sin presentación, directo en la plataforma. Cuénteme: ¿qué servicio tiene habilitado su IPS y cómo manejan hoy las listas de chequeo?"*

### Demo (15 min)
1. **Módulo auditoría** (5 min) — Mostrar lista de chequeo del servicio que mencionó. Hacer clic en 3-4 criterios. Mostrar cómo se registran hallazgos.
2. **Chat IA** (3 min) — Preguntar algo de la norma que el cliente suele consultar. Mostrar la respuesta con el artículo citado.
3. **Vencimientos** (2 min) — Crear un vencimiento de ejemplo. Mostrar la alerta.
4. **Reporte PDF** (2 min) — Generar el reporte de auditoría. Mostrar que queda listo para imprimir.
5. **Dashboard** (3 min) — Mostrar el estado general de la IPS.

### Cierre (3 min)
*"¿Qué le pareció? ¿Hay algo que le falte para su caso específico?"*

**Si hay objeciones:**
- *"Muy costoso"* → "El Starter son $299.000/mes — menos de lo que vale una hora de consultoría. Y tiene 30 días gratis para probarlo sin riesgo."
- *"Ya tenemos un sistema"* → "¿Qué sistema usan? Puedo mostrarle qué hace NormaLis diferente en habilitación."
- *"No tenemos tiempo"* → "La idea es precisamente ahorrarles tiempo. ¿Cuánto tiempo les toma hoy preparar una auditoría interna?"

**Cierre del piloto:**
*"¿Lo activo hoy? Le creo la cuenta en 5 minutos y en esta misma llamada le muestro cómo empezar."*

---

## PLAN SEMANA A SEMANA

| Semana | Acción | Meta |
|--------|--------|------|
| **1** | Lista de 50 IPS + enviar 20 WhatsApp calientes | 5 respuestas |
| **2** | 20 WhatsApp tibios + 1 post LinkedIn + 10 emails | 3 demos agendadas |
| **3** | Follow-ups + 20 WhatsApp fríos + 2do post | 3 demos realizadas |
| **4** | Cierres + post social proof + evaluar resultados | **3 pilotos activos** |

---

*Generado por NormaLis · normalis.co*
