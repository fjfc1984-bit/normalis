...
// Res. 3100/2019 (Colombia) ↔ ISO 7101:2023 ↔ Joint Commission International (JCI 8ª ed.)
// ─────────────────────────────────────────────────────────────────────────────────────────
// Cómo funciona:
//   1. Lee las respuestas de auditoría guardadas en Firestore/localStorage
//   2. Calcula el % de cumplimiento por estándar de la Res. 3100
...
// ═══════════════════════════════════════════════════════════════════
// 1. MATRIZ DE EQUIVALENCIAS (Res. 3100 → ISO 7101 → JCI)
// ═══════════════════════════════════════════════════════════════════
  ...
  container.innerHTML = `
    ...
    <div style="background:linear-gradient(135deg,#0d9488,#0369a1);border-radius:14px;padding:20px 24px;margin-bottom:20px;color:#fff">
      ...
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px">
        ${[
          { norm: '🇨🇴 Res. 3100/2019', pct: global.res3100, sub: 'Colombia — Habilitación' },
      ...
      <div style="margin-top:14px;font-size:11px;opacity:.8;background:rgba(0,0,0,.15);border-radius:8px;padding:10px">
        💡 <strong>Cómo leer esto:</strong> Al cumplir la Res. 3100, automáticamente cubres una parte de ISO 7101 y JCI. El porcentaje muestra cuánto ya tienes cubierto en cada norma gracias a tu trabajo en habilitación colombiana.
      </div>
    </div>
      ...
      ${Object.entries(scores).map(([key, s]) => `
        <div style="border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;background:#fff">
          ...
          <div style="background:#f8fafc;padding:14px 18px;display:flex;align-items:center;justify-content:space-between;cursor:pointer"
            ...
            <div style="display:flex;align-items:center;gap:10px">
              ...
              <div>
                ...
                <div style="font-size:11px;color:#64748b">${s.cumplidas} de ${s.totalPreguntas} criterios cumplidos en Res. 3100</div>
              </div>
            </div>
          ...
          <div style="padding:14px 18px;display:grid;grid-template-columns:repeat(3,1fr);gap:16px;border-bottom:1px solid #f1f5f9">
            <div>
              <div style="font-size:10px;font-weight:700;color:#475569;margin-bottom:6px;text-transform:uppercase">🇨🇴 Res. 3100</div>
    ...
    <div style="margin-top:16px;background:#f8fafc;border-radius:10px;padding:14px;font-size:11px;color:#64748b;border:1px solid #e2e8f0">
      ...
      muestran cuánto de esas normas queda cubierto al cumplir la Res. 3100, según la matriz de equivalencias NormaLis v1.0.
  ...
  if (!document.getElementById('view-crosswalk')) {
    ...
    view.innerHTML = `
      ...
      <p style="color:#64748b;font-size:13px;margin-bottom:16px">
        ...
        cumplimiento de la <strong>Resolución 3100/2019</strong>.
