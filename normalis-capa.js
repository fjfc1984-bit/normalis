      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">
        <div style="flex:1">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap">
            ...
            ${c.origen ? `<span style="background:#f1f5f9;color:#64748b;padding:2px 8px;border-radius:20px;font-size:11px">${c.origen}</span>` : ''}
          </div>
          <div style="font-size:14px;font-weight:700;color:#1e293b;margin-bottom:4px">${(c.descripcion||'').replace(/[<>]/g,'').substring(0,100)}</div>
          ${c.accionCorrectiva ? `<div style="font-size:12px;color:#475569;margin-bottom:6px">↳ ${c.accionCorrectiva.replace(/[<>]/g,'').substring(0,120)}</div>` : ''}
          <div style="display:flex;gap:16px;flex-wrap:wrap;font-size:12px;color:#94a3b8">
            ${c.responsable ? `<span>👤 ${c.responsable.replace(/[<>]/g,'')}</span>` : ''}
